...
uint32_t min_size(struct s2n_blob *blob, uint32_t max_length) {
...
int s2n_client_hello_free(struct s2n_client_hello *client_hello) {
    ...
    GUARD(s2n_free(&client_hello->cipher_suites));
    GUARD(s2n_free(&client_hello->extensions));
...
int s2n_client_hello_recv(struct s2n_connection *conn)
{
    ...
    GUARD(s2n_stuffer_read_uint8(in, &client_hello->compression_methods));
    GUARD(s2n_stuffer_skip_read(in, client_hello->compression_methods));
